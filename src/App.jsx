...
const drawRadarPDF = (doc, cx, cy, radius, themeScores) => {
  ...
  // Labels
  doc.setFontSize(7); doc.setTextColor(30, 30, 30); doc.setFont("helvetica", "bold");
  themeScores.forEach((d, i) => {
    const a = i * step - Math.PI / 2;
    const lx = cx + (radius + 16) * Math.cos(a);
    const ly = cy + (radius + 16) * Math.sin(a);
    const align = Math.abs(lx - cx) < 5 ? 'center' : lx > cx ? 'left' : 'right';
    const label = d.theme.length > 14 ? d.theme.substring(0, 13) + '…' : d.theme;
...
const drawBarChartPDF = (doc, startX, startY, chartW, barData, avgScore) => {
  ...
  // Avg line (red dashed) — draw first so bars render on top
  doc.setDrawColor(220, 38, 38); doc.setLineWidth(0.8);
  doc.setLineDashPattern([2, 2], 0);
  const totalH = barData.length * (barH + gap);
  doc.line(avgLineX, startY - 6, avgLineX, startY + totalH);
  doc.setLineDashPattern([], 0);
  // Avg label at top
  doc.setFontSize(7); doc.setTextColor(220, 38, 38); doc.setFont("helvetica", "bold");
  doc.text(`Moy. ${avgScore}`, avgLineX + 1, startY - 8);
  doc.setFont("helvetica", "normal");

  // Bars
  barData.forEach((item, i) => {
    const y = startY + i * (barH + gap);
    const bw = (barAreaW * item.score) / 5;
    const rgb = hexToRgb(getLevel(item.score).color);
    // Label
    doc.setFontSize(7); doc.setTextColor(70, 70, 70);
    const shortLabel = item.theme.length > 16 ? item.theme.substring(0, 15) + '…' : item.theme;
    doc.text(shortLabel, startX, y + barH - 1);
    // Bar
    doc.setFillColor(...rgb);
    doc.roundedRect(startX + labelW, y, bw, barH, 1.5, 1.5, 'F');
    // Score
    doc.setFontSize(7); doc.setTextColor(30, 30, 30);
    doc.text(`${item.score}`, startX + labelW + bw + 2, y + barH - 1);
  });

  // Legend
  const legendY = startY + barData.length * (barH + gap) + 6;
  let lx = startX;
...
export default function App() {
  ...
  const radarData = THEMES.map(t => ({ theme: t.length > 13 ? t.substring(0,12)+"…" : t, fullTheme: t, score: themeScore(t), fullMark: 5 }));
...
Rédige une analyse en EXACTEMENT 2 paragraphes séparés par une ligne vide.
RÈGLES ABSOLUES :
...
Propose une ou deux pistes concrètes d'amélioration.
Rappelle que cette auto-évaluation est indicative.
Invite à contacter Aravis Performance pour un audit complet ou ciblé.`;

    try {
      const res = await fetch(WORKER_AI_URL, { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ prompt }) });
  ...
  const exportResult = async () => {
    ...
    const lightBlue = [239,246,255]; const red = [220,38,38];
    ...
    doc.text("Niveau indicatif base sur un nombre reduit d'informations et sans analyse complete du perimetre supply chain.", margin, y); y += 6;

    // Niveaux de maturité avec couleurs
    doc.setFontSize(10); doc.setFont("helvetica","bold"); doc.setTextColor(...dark);
    doc.text("Les 6 niveaux de maturite Supply Chain", margin, y); y += 4;
    MATURITY_LEVELS.forEach(l => {
      const rgb = hexToRgb(l.color);
      const isCurrent = Math.round(avgScore) === l.level;
      if (isCurrent) { doc.setFillColor(...rgb); doc.roundedRect(margin, y, contentW, 10, 2, 2, "F"); }
      doc.setFillColor(...rgb); doc.circle(margin+4, y+5, 3, "F");
      doc.setFontSize(8); doc.setFont("helvetica","bold");
      doc.setTextColor(isCurrent ? 255 : 30, isCurrent ? 255 : 30, isCurrent ? 255 : 30);
      doc.text(`${l.level} — ${l.label}`, margin+10, y+5.5);
      doc.setFont("helvetica","normal");
      doc.setTextColor(isCurrent ? 240 : 80, isCurrent ? 240 : 80, isCurrent ? 240 : 80);
      doc.setFontSize(7);
      doc.text(l.desc, margin+45, y+5.5);
      if (isCurrent) { doc.setFont("helvetica","bold"); doc.text("← VOTRE NIVEAU", margin+contentW-30, y+5.5); }
      y += 11;
    });
    y += 4;
    ...
    const radarData_pdf = THEMES.map(t => ({ theme: t.length > 14 ? t.substring(0,13)+'…' : t, score: themeScore(t) }));
    const cx = pageW/2; const cy = y + 52; const radarR = 42;
    drawRadarPDF(doc, cx, cy, radarR, radarData_pdf);
    y = cy + radarR + 22;

    // Histogramme
    doc.setFontSize(10); doc.setFont("helvetica","bold"); doc.setTextColor(...dark);
    doc.text("Scores par thematique (classement)", margin, y); y += 6;
      ...
      <div style={{ maxWidth:760, margin:"0 auto", padding:"0 16px 56px" }}>
          ...
          <ResponsiveContainer width="100%" height={460}>
            <RadarChart data={radarData} margin={{ top:40,right:80,bottom:40,left:80 }}>
              ...
              <PolarAngleAxis dataKey="theme" tick={(props) => {
                ...
                words.forEach(w => { if((curr+" "+w).trim().length>10){if(curr)lines.push(curr);curr=w;}else{curr=(curr+" "+w).trim();} });
        ...
        <div style={{ background:"#fff",borderRadius:16,padding:32,boxShadow:"0 4px 24px #0001",marginBottom:20 }}>
          ...
          <ResponsiveContainer width="100%" height={340}>
            <BarChart data={barData} layout="vertical" margin={{ top:10,right:60,bottom:0,left:140 }}>
